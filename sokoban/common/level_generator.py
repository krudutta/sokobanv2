...
def level_generator_1(width, height, num_box):
	while(True):
		rm=Room(height, width, num_box)
		rm.topology_gen(int(1*(height+width)))
		...
		if score>0:
			...
			one=False
			for l in range(num_box):
				j=l
				x_b=max_config[j+l]
				y_b=max_config[j+l+1]	
				if rm.get_tile(x_b, y_b) == 'X':
					rm.set_tile(x_b, y_b,'T')
					if l == num_box-1:
						rm.set_tile(x_b, y_b,'B')
						return rm.room, tuple(rm.player_curpos)
				if rm.get_tile(x_b, y_b) == 'B' and not one:
					if l == num_box-1:
						return rm.room, tuple(rm.player_curpos)
					else:	
						rm.set_tile(x_b, y_b,'E')		
			return rm.room, tuple(rm.player_curpos)
		del rm
		del tre	
